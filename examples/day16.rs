...
impl State {
    ...
    fn make(regs: [usize; 4]) -> State {
        State { ra: regs[0], rb: regs[1], rc: regs[2], rd: regs[3] }
    }
}
...
fn fits<F>(rec: &Record, f: F) -> bool
    where F: Fn(usize, usize, usize, &[usize; 4]) -> [usize; 4] {
...
fn nfits(rec: &Record) -> Vec<usize> {
    let mut out = Vec::new();
    if fits(rec, addr) { out.push( 0); }
    if fits(rec, addi) { out.push( 1); }
    if fits(rec, mulr) { out.push( 2); }
    if fits(rec, muli) { out.push( 3); }
    if fits(rec, banr) { out.push( 4); }
    if fits(rec, bani) { out.push( 5); }
    if fits(rec, borr) { out.push( 6); }
    if fits(rec, bori) { out.push( 7); }
    if fits(rec, setr) { out.push( 8); }
    if fits(rec, seti) { out.push( 9); }
    if fits(rec, gtir) { out.push(10); }
    if fits(rec, gtri) { out.push(11); }
    if fits(rec, gtrr) { out.push(12); }
    if fits(rec, eqir) { out.push(13); }
    if fits(rec, eqri) { out.push(14); }
    if fits(rec, eqrr) { out.push(15); }
    out
}
...
fn parse_records(input: &String) -> Vec<Record> {
    ...
    for cap in re.captures_iter(input) {
        ...
        let was = State { ra: cap[1].parse().unwrap(), rb: cap[2].parse().unwrap(), rc: cap[3].parse().unwrap(), rd: cap[4].parse().unwrap() };
        let code = Code { op: cap[5].parse().unwrap(), a: cap[6].parse().unwrap(), b: cap[7].parse().unwrap(), c: cap[8].parse().unwrap() };
        let now = State { ra: cap[9].parse().unwrap(), rb: cap[10].parse().unwrap(), rc: cap[11].parse().unwrap(), rd: cap[12].parse().unwrap() };
...
fn parse_codes(input: &String) -> Vec<Code> {
    ...
    for cap in re.captures_iter(&rest) {
        let code = Code { op: cap[1].parse().unwrap(), a: cap[2].parse().unwrap(), b: cap[3].parse().unwrap(), c: cap[4].parse().unwrap() };
        result.push(code);
    }
    result
}
